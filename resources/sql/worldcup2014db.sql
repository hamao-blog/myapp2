...
/*!50001 VIEW `total_goals` AS select `p`.`id` AS `id`,count(`g`.`goal_time`) AS `goals` from (`players` `p` join `goals` `g` on((`p`.`id` = `g`.`player_id`))) where (`g`.`goal_time` is not null) group by `p`.`id` */;
...
-- Dump completed on 2020-08-29 18:01:17
